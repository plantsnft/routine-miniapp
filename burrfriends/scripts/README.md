...
## Deploy + backfill (10.3.5)For deploy workflow and optional backfill of signup profile cache, see **`BURRFRIENDS_GAME_SETUP_PHASED_PLAN_REVISED.md`** (§10.3.5, Deploy workflow step 4). From repo root with env set (e.g. `.env.local`), run `npm run backfill:signup-profiles` to prefill `username`/`display_name`/`pfp_url` on existing `buddy_up_signups` and `mole_signups`; script: `scripts/backfill-signup-profile-cache.ts`.