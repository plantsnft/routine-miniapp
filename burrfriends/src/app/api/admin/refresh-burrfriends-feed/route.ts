/**
 * POST /api/admin/refresh-burrfriends-feed
 * Manually trigger Burrfriends channel feed refresh (admin only)
 * 
 * This endpoint allows admins to manually refresh the feed cache
 * without waiting for the next cron run. Useful for:
 * - Initial setup/testing
 * - Manual refresh when needed
 * - Troubleshooting
 * 
 * SAFETY: Requires admin authentication
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ADMIN CHECK: Only allow admins
    if (!isAdmin(fid)) {
      console.warn("[Admin Refresh Burrfriends Feed] Unauthorized access attempt", { fid });
    ...
    try {
      ...
      if (!feedResponse.ok) {
        ...
        console.error(`[Admin Refresh Burrfriends Feed] Feed API error: ${feedResponse.status} ${errorText}`);
        structuredError = {
          code: "FEED_API_ERROR",
        ...
        console.log(`[Admin Refresh Burrfriends Feed] Fetched ${casts.length} casts from feed API`);
      }
      ...
      try {
        ...
        if (channel) {
          ...
          console.log(`[Admin Refresh Burrfriends Feed] Channel stats: ${JSON.stringify(channelStats)}`);
        }
      } catch (lookupError: any) {
        console.warn("[Admin Refresh Burrfriends Feed] Channel lookup failed, trying search:", lookupError?.message);
          ...
          if (searchResponse.ok) {
            ...
            if (searchData.channels && Array.isArray(searchData.channels)) {
              ...
              if (foundChannel) {
                ...
                console.log(`[Admin Refresh Burrfriends Feed] Channel stats from search: ${JSON.stringify(channelStats)}`);
              }
            }
          }
        } catch (searchError) {
          console.warn("[Admin Refresh Burrfriends Feed] Channel search also failed:", searchError);
        }
      }
    } catch (err) {
      console.error("[Admin Refresh Burrfriends Feed] Error fetching feed:", err);
      structuredError = {
        code: "FETCH_ERROR",
    ...
    try {
      ...
      console.log(`[Admin Refresh Burrfriends Feed] Successfully cached ${casts.length} casts`);
    } catch (err) {
      supabaseError = err;
      console.error("[Admin Refresh Burrfriends Feed] Supabase error:", err);
    }
    ...
    if (!response.ok) {
      return NextResponse.json<ApiResponse>(response, { status: 500 });
    }
    ...
    console.error("[Admin Refresh Burrfriends Feed] Unexpected error:", error);
    return NextResponse.json<ApiResponse>(
      { ok: false, error: error?.message || "Failed to refresh feed" },
      { status: 500 }
    );
  }
}
