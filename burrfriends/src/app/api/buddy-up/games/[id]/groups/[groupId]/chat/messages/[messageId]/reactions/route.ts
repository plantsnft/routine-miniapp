/**
 * POST /api/buddy-up/games/[id]/groups/[groupId]/chat/messages/[messageId]/reactions
 * Set, change, or remove the current user's reaction on a message (one per user per message).
 * Same access as group chat: in group or isAdmin.
 */
...
export async function POST(
  ...
  try {
    ...
    const messages = await pokerDb.fetch<{ id: string; group_id: string }>("buddy_up_chat_messages", {
    ...
    const existing = await pokerDb.fetch<{ reaction: string }>("buddy_up_chat_reactions", {
      filters: { message_id: messageId, fid },
      limit: 1,
    });

    if (existing && existing.length > 0) {
      const current = existing[0].reaction;
      if (current === reaction) {
        // Toggle off: remove reaction
        await pokerDb.delete("buddy_up_chat_reactions", { message_id: messageId, fid });
        return NextResponse.json<ApiResponse>({
          ok: true,
          data: { reaction: null, counts: await getCounts(messageId) },
        });
      }
      // Replace: update row (delete + insert since we use composite PK)
      await pokerDb.delete("buddy_up_chat_reactions", { message_id: messageId, fid });
    }

    await pokerDb.insert("buddy_up_chat_reactions", [
    ...
    console.error("[buddy-up/chat/messages/[messageId]/reactions POST]", e);
...
async function getCounts(messageId: string): Promise<{ thumbs_up: number; x: number; fire: number; scream: number }> {
  const rows = await pokerDb.fetch<{ reaction: string }>("buddy_up_chat_reactions", {
