/**
 * POST /api/bullied/games/[id]/groups/[groupId]/chat/messages/[messageId]/reactions
...
export async function POST(
  ...
  try {
    ...
    const messages = await pokerDb.fetch<{ id: string; group_id: string }>("bullied_chat_messages", {
    ...
    const existing = await pokerDb.fetch<{ reaction: string }>("bullied_chat_reactions", {
    ...
    if (existing && existing.length > 0) {
      if (existing[0].reaction === reaction) {
        await pokerDb.delete("bullied_chat_reactions", { message_id: messageId, fid });
    ...
    await pokerDb.insert("bullied_chat_reactions", [
    ...
    console.error("[bullied/chat/messages/[messageId]/reactions POST]", e);
...
async function getCounts(messageId: string): Promise<{ thumbs_up: number; x: number; fire: number; scream: number }> {
  const rows = await pokerDb.fetch<{ reaction: string }>("bullied_chat_reactions", {
