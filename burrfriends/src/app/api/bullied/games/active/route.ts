/**
 * GET /api/bullied/games/active - Get active games (open + in_progress), excluding previews
 */
...
export async function GET(_req: NextRequest) {
  try {
    const openGames = await pokerDb.fetch<any>("bullied_games", {
      filters: { status: "open", community: "betr" }, // Phase 36: user-facing only sees BETR
      order: "created_at.desc",
      limit: 10,
    });

    const inProgress = await pokerDb.fetch<any>("bullied_games", {
    ...
    const filtered = allActive.filter((g: any) => g.is_preview !== true);

    return NextResponse.json<ApiResponse>({ ok: true, data: filtered });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("[bullied/games/active GET]", e);
