  ...
  try {
    ...
    try {
      ...
      if (insertError?.message?.includes('Could not find the table') || 
          insertError?.message?.includes('game_requests') ||
          insertError?.code === 'PGRST205') {
        return NextResponse.json<ApiResponse>(
          { ok: false, error: "Game requests feature is not available yet. Please contact an admin." },
          { status: 503 }
        );
      }
      throw insertError;
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Also handle case where table doesn't exist yet (return empty array)
    let requests: any[] = [];
    try {
      ...
      if (fetchError?.message?.includes('Could not find the table') || 
          fetchError?.message?.includes('game_requests') ||
          fetchError?.code === 'PGRST205') {
        safeLog('info', '[game-requests] Table does not exist yet, returning empty array', {
          correlationId,
          adminFid: fid,
          status,
        });
