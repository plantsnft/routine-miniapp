...
export async function PATCH(
  ...
  try {
    ...
    const gameRes = await fetch(
      `${supabaseUrl}/rest/v1/burrfriends_games?id=eq.${gameId}&select=*,club:clubs!inner(owner_fid)`,
      { 
        headers: {
    ...
    const updateRes = await fetch(
      `${supabaseUrl}/rest/v1/burrfriends_participants?game_id=eq.${gameId}&fid=eq.${playerFid}`,
      {
        method: "PATCH",
        headers: {
          ...SUPABASE_SERVICE_HEADERS,
    ...
    const fetchRes = await fetch(
      `${supabaseUrl}/rest/v1/burrfriends_participants?game_id=eq.${gameId}&fid=eq.${playerFid}&select=*`,
      { 
        headers: {
          ...SUPABASE_SERVICE_HEADERS,
          'Accept-Profile': 'poker',
