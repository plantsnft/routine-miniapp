...
export async function POST(
  ...
  try {
    ...
    const parsedFid = parseInt(String(targetFid), 10);
    if (Number.isNaN(parsedFid) || parsedFid < 1) {
      return NextResponse.json<ApiResponse>(
    ...
    // Reserve a spot: only when game is open and (if max set) under capacity
    ...
    if (game.status !== 'open' && game.status !== 'in_progress') {
      return NextResponse.json<ApiResponse>(
        { ok: false, error: `Adding participants is only allowed for open or in-progress games. This game is ${game.status}.` },
        { status: 400 }
      );
    }
    ...
    const alreadyJoined = existingParticipants.find((p: any) => (p.fid ?? (p as any).player_fid) === parsedFid);
    if (alreadyJoined) {
      return NextResponse.json<ApiResponse<GameParticipant>>({
        ok: true,
        data: alreadyJoined as GameParticipant,
      });
    }

    const joinedCount = existingParticipants.filter((p: any) => p.status === 'joined').length;
    const maxParticipants = game.max_participants;
    if (maxParticipants != null && typeof maxParticipants === 'number' && joinedCount >= maxParticipants) {
      return NextResponse.json<ApiResponse>(
        { ok: false, error: "Game is full. Remove a participant first to reserve a spot." },
        { status: 400 }
      );
    }
