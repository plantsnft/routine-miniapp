...
export async function POST(
  ...
  try {
    ...
    try {
      await pokerDb.delete('burrfriends_game_results', { game_id: gameId });
    ...
    const savedResults = await pokerDb.insert('burrfriends_game_results', resultsData as any);
    ...
    // Phase 4: Update stats for all players after results are submitted
    try {
      const { updatePlayerStats } = await import('~/lib/stats');
      
      // Get all unique FIDs from results to update their stats
      const uniqueFids = Array.from(new Set(results.map((r: any) => r.player_fid)));
      for (const playerFid of uniqueFids) {
        try {
          await updatePlayerStats(Number(playerFid));
        } catch (statsError: any) {
          // Log but don't fail results submission if stats update fails
          console.warn('[results] Failed to update stats for player', {
            gameId,
            fid: playerFid,
      ...
      // Log but don't fail results submission if stats update fails
      console.warn('[results] Failed to update player stats', {
        gameId,
