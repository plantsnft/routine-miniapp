...
export async function GET(
  ...
  try {
    ...
    try {
      const viewerParticipants = await pokerDb.fetch('burrfriends_participants', {
    ...
    if (settleTxHash && (gamesRaw[0].status === 'settled' || gamesRaw[0].status === 'completed')) {
      try {
        const withPayout = await pokerDb.fetch<any>('burrfriends_participants', {
...
export async function PATCH(
  ...
  try {
    ...
    const existingGames = await pokerDb.fetch<any>('burrfriends_games', {
      filters: { id: gameId },
      select: '*',
      limit: 1,
    });
    ...
    const updatedGames = await pokerDb.update<Game>('burrfriends_games',
      { id: gameId },
      updateData
    );
    ...
    if (isPaidGame && (trigger_contract_redeploy || entryFeeChanged || currencyChanged)) {
      try {
        ...
        if (checkIsPaidGame(game) && game.entry_fee_amount) {
          ...
          // Refresh game data
          const refreshedGames = await pokerDb.fetch<any>('burrfriends_games', {
            filters: { id: gameId },
            select: '*',
            limit: 1,
          });
