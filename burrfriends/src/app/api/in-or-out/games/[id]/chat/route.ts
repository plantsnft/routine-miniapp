/**
 * GET /api/in-or-out/games/[id]/chat - Get game-level chat messages
 * POST /api/in-or-out/games/[id]/chat - Send message
 * Access: eligible for game (alive or admin preview bypass) or isAdmin.
 */
...
export async function GET(
  ...
  try {
    ...
    const reactionsList = messageIds.length > 0
      ? await pokerDb.fetch<{ message_id: string; fid: number; reaction: string }>("in_or_out_chat_reactions", {
    ...
    console.error("[in-or-out/games/[id]/chat GET]", e);
...
export async function POST(
  ...
  try {
    ...
    console.error("[in-or-out/games/[id]/chat POST]", e);
