/**
 * POST /api/kill-or-keep/games/[id]/chat/messages/[messageId]/reactions
...
export async function POST(
  ...
  try {
    ...
    const messages = await pokerDb.fetch<{ id: string; game_id: string }>("kill_or_keep_chat_messages", {
    ...
    const existing = await pokerDb.fetch<{ reaction: string }>("kill_or_keep_chat_reactions", {
    ...
    if (existing && existing.length > 0) {
      if (existing[0].reaction === reaction) {
        await pokerDb.delete("kill_or_keep_chat_reactions", { message_id: messageId, fid });
    ...
    await pokerDb.insert("kill_or_keep_chat_reactions", [
    ...
    console.error("[kill-or-keep/chat/messages/[messageId]/reactions POST]", e);
...
async function getCounts(messageId: string): Promise<{ thumbs_up: number; x: number; fire: number; scream: number }> {
  const rows = await pokerDb.fetch<{ reaction: string }>("kill_or_keep_chat_reactions", {
