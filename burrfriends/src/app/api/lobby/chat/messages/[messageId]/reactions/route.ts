/**
 * POST /api/lobby/chat/messages/[messageId]/reactions
 * Set, change, or remove the current user's reaction on a lobby message. Requires 1M+ BETR staked.
 */
...
const LOBBY_CHAT_MIN_STAKE = 1_000_000;
...
export async function POST(
  ...
  try {
    ...
    const messages = await pokerDb.fetch<{ id: string }>("lobby_chat_messages", {
    ...
    const existing = await pokerDb.fetch<{ reaction: string }>("lobby_chat_reactions", {
    ...
    if (existing && existing.length > 0) {
      if (existing[0].reaction === reaction) {
        await pokerDb.delete("lobby_chat_reactions", { message_id: messageId, fid });
        return NextResponse.json<ApiResponse>({
          ok: true,
    ...
    await pokerDb.insert("lobby_chat_reactions", [
    ...
    console.error("[lobby/chat/messages/[messageId]/reactions POST]", e);
...
async function getCounts(messageId: string): Promise<{ thumbs_up: number; x: number; fire: number; scream: number }> {
  const rows = await pokerDb.fetch<{ reaction: string }>("lobby_chat_reactions", {
