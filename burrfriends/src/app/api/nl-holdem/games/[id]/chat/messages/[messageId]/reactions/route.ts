/**
 * POST /api/nl-holdem/games/[id]/chat/messages/[messageId]/reactions
 * Set, change, or remove the current user's reaction on a message.
 * Phase 40.
 */
...
async function canAccessGameChat(fid: number, gameId: string, req: NextRequest): Promise<boolean> {
  ...
  const games = await pokerDb.fetch<{ id: string; is_preview?: boolean; seat_order_fids?: number[] | null }>("nl_holdem_games", {
    filters: { id: gameId },
    limit: 1,
  });
  ...
  return (signups || []).some((s) => Number(s.fid) === fid);
}

async function getCounts(messageId: string): Promise<{ thumbs_up: number; x: number; fire: number; scream: number }> {
  const rows = await pokerDb.fetch<{ reaction: string }>("nl_holdem_chat_reactions", {
...
export async function POST(
  ...
  try {
    ...
    const messages = await pokerDb.fetch<{ id: string; game_id: string }>("nl_holdem_chat_messages", {
    ...
    const existing = await pokerDb.fetch<{ reaction: string }>("nl_holdem_chat_reactions", {
    ...
    if (existing && existing.length > 0) {
      if (existing[0].reaction === reaction) {
        await pokerDb.delete("nl_holdem_chat_reactions", { message_id: messageId, fid });
    ...
    await pokerDb.insert("nl_holdem_chat_reactions", [{ message_id: messageId, fid, reaction }]);
    ...
    console.error("[nl-holdem/chat/messages/[messageId]/reactions POST]", e);
