/**
 * GET /api/nl-holdem/games/[id]/chat - Get game chat messages
 * POST /api/nl-holdem/games/[id]/chat - Send message
 * Access: in signups or seat_order_fids or admin. Phase 40.
 */
...
export async function GET(
  ...
  try {
    ...
    const reactionsList =
      messageIds.length > 0
        ? await pokerDb.fetch<{ message_id: string; fid: number; reaction: string }>("nl_holdem_chat_reactions", {
    ...
    console.error("[nl-holdem/games/[id]/chat GET]", e);
...
export async function POST(
  ...
  try {
    ...
    const inserted = await pokerDb.insert("nl_holdem_chat_messages", [
    ...
    console.error("[nl-holdem/games/[id]/chat POST]", e);
