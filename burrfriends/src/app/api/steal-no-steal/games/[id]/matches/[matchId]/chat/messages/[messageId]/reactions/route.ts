/**
 * POST /api/steal-no-steal/games/[id]/matches/[matchId]/chat/messages/[messageId]/reactions
 * Set, change, or remove the current user's reaction on a message. Same access as match chat.
 */
...
export async function POST(
  req: NextRequest,
  { params }: { params: Promise<{ id: string; matchId: string; messageId: string }> }
) {
  try {
    ...
    const matches = await pokerDb.fetch<{ id: string; player_a_fid: number; player_b_fid: number }>("steal_no_steal_matches", {
      filters: { id: matchId },
      limit: 1,
    });
    ...
    if (!isPlayerInMatch && !isAdmin(fid)) {
      return NextResponse.json<ApiResponse>({ ok: false, error: "Not authorized" }, { status: 403 });
    }

    const messages = await pokerDb.fetch<{ id: string; match_id: string }>("steal_no_steal_chat_messages", {
      filters: { id: messageId },
      limit: 1,
    });
    if (!messages || messages.length === 0 || messages[0].match_id !== matchId) {
      return NextResponse.json<ApiResponse>({ ok: false, error: "Message not found" }, { status: 404 });
    }

    const existing = await pokerDb.fetch<{ reaction: string }>("steal_no_steal_chat_reactions", {
    ...
    if (existing && existing.length > 0) {
      if (existing[0].reaction === reaction) {
        await pokerDb.delete("steal_no_steal_chat_reactions", { message_id: messageId, fid });
    ...
    await pokerDb.insert("steal_no_steal_chat_reactions", [
    ...
    console.error("[steal-no-steal/chat/messages/[messageId]/reactions POST]", e);
...
async function getCounts(messageId: string): Promise<{ thumbs_up: number; x: number; fire: number; scream: number }> {
  const rows = await pokerDb.fetch<{ reaction: string }>("steal_no_steal_chat_reactions", {
