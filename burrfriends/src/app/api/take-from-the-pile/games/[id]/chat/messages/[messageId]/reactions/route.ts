/**
 * POST /api/take-from-the-pile/games/[id]/chat/messages/[messageId]/reactions
...
async function canAccessGameChat(fid: number, gameId: string, req: NextRequest): Promise<boolean> {
  ...
  const games = await pokerDb.fetch<{ id: string; is_preview?: boolean }>("take_from_the_pile_games", {
    filters: { id: gameId },
    limit: 1,
  });
...
export async function POST(
  ...
  try {
    ...
    const messages = await pokerDb.fetch<{ id: string; game_id: string }>("take_from_the_pile_chat_messages", {
    ...
    const existing = await pokerDb.fetch<{ reaction: string }>("take_from_the_pile_chat_reactions", {
    ...
    if (existing && existing.length > 0) {
      if (existing[0].reaction === reaction) {
        await pokerDb.delete("take_from_the_pile_chat_reactions", { message_id: messageId, fid });
    ...
    await pokerDb.insert("take_from_the_pile_chat_reactions", [
    ...
    console.error("[take-from-the-pile/chat/messages/[messageId]/reactions POST]", e);
...
async function getCounts(messageId: string): Promise<{ thumbs_up: number; x: number; fire: number; scream: number }> {
  const rows = await pokerDb.fetch<{ reaction: string }>("take_from_the_pile_chat_reactions", {
