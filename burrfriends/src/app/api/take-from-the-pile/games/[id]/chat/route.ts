/**
 * GET /api/take-from-the-pile/games/[id]/chat - Get game-level chat messages
 * POST /api/take-from-the-pile/games/[id]/chat - Send message
 * Access: eligible (alive community betr or admin preview bypass) or isAdmin. Phase 37.
 */
...
export async function GET(
  ...
  try {
    ...
    const reactionsList = messageIds.length > 0
      ? await pokerDb.fetch<{ message_id: string; fid: number; reaction: string }>("take_from_the_pile_chat_reactions", {
    ...
    console.error("[take-from-the-pile/games/[id]/chat GET]", e);
...
export async function POST(
  ...
  try {
    ...
    console.error("[take-from-the-pile/games/[id]/chat POST]", e);
