/**
 * GET /api/the-mole/games/[id]/groups/[groupId]/chat - Get chat messages for a group
 * POST /api/the-mole/games/[id]/groups/[groupId]/chat - Send a message to group chat
 * Authorization: User must be in the group OR be an admin
 * 
 * OPTIMIZATION (§15.11.1): Reads sender profiles from mole_signups table instead of Neynar.
...
export async function GET(
  ...
  try {
    ...
    const messages = await pokerDb.fetch<{ id: string; sender_fid: number; message: string; created_at: string }>("mole_chat_messages", {
    ...
    const reactionsList = messageIds.length > 0
      ? await pokerDb.fetch<{ message_id: string; fid: number; reaction: string }>("mole_chat_reactions", {
    ...
    console.error("[the-mole/.../chat GET]", e);
...
export async function POST(
  ...
  try {
    ...
    const groups = await pokerDb.fetch<{ id: string; round_id: string; status: string }>("mole_groups", {
      filters: { id: groupId },
      limit: 1,
    });
    if (!groups || groups.length === 0) {
    ...
    const inserted = await pokerDb.insert(
      "mole_chat_messages",
      [{ group_id: groupId, sender_fid: fid, message, created_at: now, updated_at: now }],
      "id"
    );
    ...
    console.error("[the-mole/.../chat POST]", e);
