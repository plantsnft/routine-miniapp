...
export type AuthStatus = 'loading' | 'authed' | 'error' | 'blocked';
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const verifyToken = useCallback(async (authToken: string) => {
    ...
    try {
      ...
      if (data.blocked) {
        if (DEBUG) console.log('[AuthProvider] User is blocked');
        setStatus('blocked');
        return false;
      }
  ...
  // Phase 20: Show blocked message if user is blocked
  if (status === 'blocked') {
    return (
      <div style={{ 
        display: 'flex', 
        ...
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '1rem' }}>
            You are blocked from this mini app
          </h1>
        </div>
      </div>
    );
