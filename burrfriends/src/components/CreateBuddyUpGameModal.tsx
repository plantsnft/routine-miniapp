...
interface CreateBuddyUpGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export function CreateBuddyUpGameModal({
  isOpen,
  onClose,
  onSuccess,
}: CreateBuddyUpGameModalProps) {
  const { token } = useAuth();
  const [prizeAmount, setPrizeAmount] = useState('');
  const [stakingMinAmount, setStakingMinAmount] = useState<number | null>(null);
  const [startCondition, setStartCondition] = useState<'min_players' | 'at_time' | 'whichever_first' | ''>('');
  const [minPlayersToStart, setMinPlayersToStart] = useState<number>(3);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (startCondition === 'at_time' || startCondition === 'whichever_first') {
      if (!signupClosesAt) {
    ...
    if (startCondition === 'min_players' || startCondition === 'whichever_first') {
    ...
    try {
      const res = await authedFetch('/api/buddy-up/games', {
        method: 'POST',
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            >
              ...
              <option value="whichever_first">When N players or at time, whichever first</option>
