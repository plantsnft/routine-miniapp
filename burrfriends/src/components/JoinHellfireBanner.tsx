      ...
      if (currentItem.title === 'Request to join the group chat') {
        try {
          ...
          if (sdk?.actions?.composeCast) {
            // Use the specific SIAs Poker Room games page URL
            const { HELLFIRE_CLUB_SLUG } = await import('~/lib/constants');
            const miniAppUrl = `https://poker-swart.vercel.app/clubs/${HELLFIRE_CLUB_SLUG}/games`;
            await sdk.actions.composeCast({
              text: 'i would like to join the poker lobby.. view the mini app here to get in on the action',
              embeds: [miniAppUrl],
            });
          }
        } catch (error) {
