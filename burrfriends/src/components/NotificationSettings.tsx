            ...
            <div>
              ...
              <input
                ...
                onPaste={async (e) => {
                  ...
                  const end = el.selectionEnd ?? broadcastTitle.length;
                  const text = await getPasteText(e);
                  if (text != null && text !== '') {
                    e.preventDefault();
                    setBroadcastTitle((prev) => (prev.slice(0, start) + text + prev.slice(end)).slice(0, 32));
                  }
                }}
                maxLength={32}
                className="input"
            ...
            <div>
              ...
              <textarea
                ...
                onPaste={async (e) => {
                  const el = e.currentTarget;
                  const start = el.selectionStart ?? 0;
                  const end = el.selectionEnd ?? broadcastBody.length;
                  const text = await getPasteText(e);
                  if (text != null && text !== '') {
                    e.preventDefault();
                    setBroadcastBody((prev) => (prev.slice(0, start) + text + prev.slice(end)).slice(0, 128));
                  }
                }}
                maxLength={128}
                rows={3}
            ...
            <div>
              ...
              <input
                ...
                onPaste={async (e) => {
                  const el = e.currentTarget;
                  const start = el.selectionStart ?? 0;
                  const end = el.selectionEnd ?? broadcastTargetUrl.length;
                  const text = await getPasteText(e);
                  if (text != null && text !== '') {
                    e.preventDefault();
                    setBroadcastTargetUrl((prev) => prev.slice(0, start) + text + prev.slice(end));
                  }
                }}
                className="input"
                style={{ color: 'var(--text-primary)' }}
