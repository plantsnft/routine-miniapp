...
/**
 * Convert human-readable BETR amount to token units (18 decimals)
 * @param betr Human-readable amount (e.g., "0.1", "1.5")
 * @returns Amount in token units as string
 */
export function betrToUnits(betr: string | number): string {
  const amount = typeof betr === 'string' ? parseFloat(betr) : betr;
  if (isNaN(amount) || amount < 0) {
    throw new Error(`Invalid BETR amount: ${betr}`);
  }
  // Multiply by 10^18 and ensure no decimal places
  const units = BigInt(Math.floor(amount * 10 ** BETR_DECIMALS));
  return units.toString();
}

/**
 * Convert BETR token units to human-readable amount
 * @param units Amount in token units
 * @returns Human-readable amount with up to 18 decimal places
 */
export function unitsToBetr(units: string | bigint): string {
  const unitsBigInt = typeof units === 'string' ? BigInt(units) : units;
  const betr = Number(unitsBigInt) / 10 ** BETR_DECIMALS;
  return betr.toFixed(BETR_DECIMALS).replace(/\.?0+$/, '');
}
...
export function amountToUnits(
  ...
  currency: 'ETH' | 'USDC' | 'BASE_ETH' | 'BETR'
): string {
  if (currency === 'USDC') {
    return usdcToUnits(amount);
  } else if (currency === 'BETR') {
    // BETR uses 18 decimals (same as ETH)
    return betrToUnits(amount);
  } else {
...
export function validateAmount(
  ...
  // For BETR, enforce max of 100 million if not explicitly provided
  const effectiveMax = max !== undefined 
    ? max 
    : (currency === 'BETR' ? BETR_MAX_AMOUNT : undefined);

  if (effectiveMax !== undefined && numAmount > effectiveMax) {
    return { valid: false, error: `Amount cannot exceed ${effectiveMax} ${currency}` };
  }
