 ...
 * BETR → BETR_TOKEN_ADDRESS
 */
export function mapCurrencyToAddress(currency: string): string {
  ...
  if (upperCurrency === 'BETR') {
    return BETR_TOKEN_ADDRESS;
  }
  throw new Error(`Unsupported currency: ${currency}. Supported: ETH, BASE_ETH, USDC, BETR`);
}
...
export async function createGameOnContract(
  ...
  // Convert amount to contract units (wei for ETH, 6 decimals for USDC, 18 decimals for BETR)
  const entryFeeUnits = amountToUnits(amount.toString(), entryFeeCurrency as 'ETH' | 'USDC' | 'BASE_ETH' | 'BETR');
...
export async function verifyCreateGameTransaction(
  ...
  try {
    ...
    const expectedEntryFeeUnits = amountToUnits(expectedEntryFee, expectedCurrency as 'ETH' | 'USDC' | 'BASE_ETH' | 'BETR');
