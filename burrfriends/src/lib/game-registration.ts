...
export function computeRegistrationCloseAt(game: {
  ...
  if (!startTime) {
    return null; // No start time = registration open until full (no late registration)
  }
...
export function hasGameStarted(
  ...
  const startTime = game.scheduled_time || game.game_date;
  if (!startTime) {
    return false; // No start time = not started (game starts when table is full)
  }
...
export function getGameBadges(
  ...
  try {
    // If registrationOpen is provided, use it but also compute full status to get reason
    if (game.registrationOpen !== undefined) {
      // Get full status to include reason (needed for badge logic)
      const fullStatus = isRegistrationOpen(game, currentParticipantCount, now);
      registrationStatus = {
        isOpen: game.registrationOpen, // Use enriched field (authoritative)
        closeAt: game.registrationCloseAt ?? null,
        reason: fullStatus.reason, // Include reason for badge logic
      };
    } else {
      registrationStatus = isRegistrationOpen(game, currentParticipantCount, now);
    }
  } catch (err) {
  ...
  if (status === 'settled') {
    ...
    // For games without start time: if full, show "Registration closed", otherwise "Registration open"
    const hasNoStartTime = !game.scheduled_time && !game.game_date;
    if (hasNoStartTime && !registrationStatus.isOpen) {
      // Game without start time that is full or closed - show "Registration closed"
      primaryLabel = 'Registration closed';
    } else if (registrationStatus.isOpen) {
      primaryLabel = 'Registration open';
    } else {
      // Registration closed for other reasons (not full, but closed)
      primaryLabel = 'Registration closed';
    }
  }
