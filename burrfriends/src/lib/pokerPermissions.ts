...
/**
 * Validate that a club is BETR WITH BURR (MVP-only restriction)
 * Throws error if club is not BETR WITH BURR
 * 
 * During migration: Accepts both old "hellfire" and new "burrfriends" slugs
 */
export async function requireHellfireClub(clubIdOrSlug: string): Promise<void> {
  try {
    ...
    // Slugs are short strings like "burrfriends"
    ...
    // Accept both old and new slug during migration period
    const validSlugs = [HELLFIRE_CLUB_SLUG, 'hellfire'];
    const clubSlug = (club[0] as any).slug;
    
    if (!validSlugs.includes(clubSlug)) {
      throw new Error('Only BETR WITH BURR club is supported in MVP');
    }
  } catch (error: any) {
    if (error.message?.includes('BETR WITH BURR') || error.message?.includes('Club not found')) {
      throw error;
    }
    throw new Error('Club not found');
  }
...
export async function getClubForGame(gameId: string): Promise<string | null> {
  try {
    const games = await pokerDb.fetch('burrfriends_games', {
      select: 'club_id',
      filters: { id: gameId },
      limit: 1,
    });
