...
export interface Game {
  ...
  // Prize configuration (for prize-based games)
  prize_amounts?: number[] | null; // Array of prize amounts (e.g., [3000000, 2000000, 1000000] for top 3)
  prize_currency?: string | null; // Currency for prizes (default: BETR)
  number_of_winners?: number | null; // Number of winners (must match prize_amounts array length)
  total_prize_amount?: number | null; // Computed: sum of all prize_amounts
  ...
  apply_staking_multipliers?: boolean | null; // Apply staking tier multipliers (1x-5x) for tournament prize payouts. Default: true. Mutually exclusive with double_payout_if_bb.
  double_payout_if_bb?: boolean | null; // Double payout if Betr Believer (50M+ staked) for tournament prize payouts. Default: false. Mutually exclusive with apply_staking_multipliers.
  // Payment contract
  ...
  is_pinned?: boolean | null; // When true, game appears first in BETR POKER list (admin-only)
  inserted_at: string;
  updated_at: string;
}
