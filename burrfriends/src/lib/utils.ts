...
/**
 * Format countdown time until a future date (e.g., "2h 30m", "1d 5h", "45m")
 * Returns empty string if date is in the past or invalid
 * Used for notification bodies (must be <= 128 chars per Farcaster spec)
 */
 ...
 * - Splash screen: splashImageUrl (using icon.png temporarily), splashBackgroundColor
 ...
 * 
 * Note: splashImageUrl currently uses icon.png (1024x1024) instead of ideal 200x200, but works for Developer Portal requirement.
 * 
...
export async function getFarcasterDomainManifest(
  ...
  const miniappConfig: any = {
    version: '1' as const,
    name: process.env.APP_NAME || 'BETR WITH BURR',
    homeUrl: appUrl,
    iconUrl,
    // Include description (default if not set)
    description: process.env.APP_DESCRIPTION || 'play poker with burr and friends',
    // Splash screen (required by Developer Portal)
    // Using icon.png temporarily (1024x1024) - ideally should be 200x200, but this works for now
    splashImageUrl: `${appUrl}/icon.png`,
    splashBackgroundColor: process.env.APP_SPLASH_BACKGROUND_COLOR || '#000000',
