...
export async function POST(req: NextRequest) {
  try {
    ...
    const quickParticipantCheck = await pokerDb.fetch<GameParticipant>('participants', {
    ...
    const joinedParticipantsForRegCheck = await pokerDb.fetch<any>('participants', {
    ...
    const existingByTxHash = await pokerDb.fetch<GameParticipant>('participants', {
    ...
    const expectedAmount = amountToUnits(game.entry_fee_amount.toString(), currency as 'ETH' | 'USDC');
    ...
    // 3. tx recipient/contract address matches expected (GameEscrow contract)
    ...
    if (process.env.ENABLE_PUSH_NOTIFICATIONS === 'true') {
      try {
        ...
        if (maxParticipants !== null && maxParticipants !== undefined && gameStatus === 'open') {
          ...
          const allParticipants = await pokerDb.fetch<any>('participants', {
