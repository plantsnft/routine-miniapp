...
import { BASE_CHAIN_ID, GAME_ESCROW_CONTRACT, BASE_USDC_ADDRESS } from '~/lib/constants';
...
export function PaymentButton({ game, playerFid, onSuccess, onError, compact = false, buttonRef, customText }: PaymentButtonProps) {
  ...
  const handlePayment = async (e?: MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      // For USDC, check allowance and approve if needed (automatically proceed to payment)
      if (!isETH) {
        setStep('approving');
        ...
        try {
          const currentAllowance = await usdcContract.allowance(userAddress, GAME_ESCROW_CONTRACT);
          const requiredAmount = BigInt(amountWei);
          const allowanceBefore = currentAllowance.toString();
          
          console.log('[PaymentButton] Allowance check:', {
            allowanceBefore,
            entryFee: amountWei,
          ...
          if (currentAllowance < requiredAmount) {
            ...
            console.log('[PaymentButton] USDC approval sent:', approveTxHash);
            ...
            try {
              ...
              while (Date.now() - startTime < maxWaitTime) {
                const polledAllowance = await usdcContract.allowance(userAddress, GAME_ESCROW_CONTRACT);
              ...
              // Final check
              const finalAllowance = await usdcContract.allowance(userAddress, GAME_ESCROW_CONTRACT);
              if (finalAllowance < requiredAmount) {
                throw new Error('Approval transaction did not update allowance within timeout');
              }
            }
            
            const allowanceAfter = (await usdcContract.allowance(userAddress, GAME_ESCROW_CONTRACT)).toString();
            ...
            console.log('[PaymentButton] USDC allowance already sufficient, skipping approval');
          }
        } catch (approveErr: any) {
          if (approveErr?.code === 4001 || approveErr?.message?.includes('rejected') || approveErr?.message?.includes('User rejected')) {
            throw new Error('Payment was cancelled');
          }
          throw new Error(`USDC approval failed: ${approveErr?.message || 'Unknown error'}`);
        }
      }
